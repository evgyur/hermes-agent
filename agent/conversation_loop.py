...
def _should_use_streaming_api(agent: Any) -> bool:
    ...
    # MoA only streams when a display/TTS consumer can receive deltas. With
    # no consumers its facade intentionally returns a complete response.
    if provider == "moa" and not agent._has_stream_consumers():
        return False
