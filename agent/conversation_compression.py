...
def compress_context(
    ...
    *,
    ...
    try:
        ...
        # A store containing records is authoritative even when every item is
        # completed/cancelled and format_for_injection() returns no active
        # snapshot. A truly empty/unreadable legacy store is not authoritative:
        # after a cold resume the retained snapshot may be the only surviving
        # record of pending work, so fail conservative and preserve it.
        ...
        _retire_stale_todos = bool(todo_snapshot) or _todo_store_is_authoritative
        if _retire_stale_todos:
            _without_stale_todos = []
            _removed_standalone_snapshot = False
            for _todo_message in compressed:
                if not isinstance(_todo_message, dict):
                    _without_stale_todos.append(_todo_message)
                    continue
                if (
                    _todo_message.get("role") == "system"
                    and _message_text(_todo_message).lstrip().startswith(
                        TODO_INJECTION_HEADER
                    )
                ):
                    _removed_standalone_snapshot = True
                    continue
                if _todo_message.get("role") != "user":
                    _without_stale_todos.append(_todo_message)
                    continue
                ...
                if _todo_stripped == _todo_content:
                    _without_stale_todos.append(_todo_message)
                    continue
                if _todo_snapshot_is_only_content(
                ...
                _without_stale_todos.append(_todo_message)
            compressed = _without_stale_todos
            if _removed_standalone_snapshot:
                # Removing a non-tail standalone snapshot may expose adjacent
                # assistant turns. Reuse the normal replay repair so content
                # and tool-call metadata stay intact.
