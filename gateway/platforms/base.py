...
_SUPERGOAL_REVIEW_FILENAMES = frozenset(
...
def _is_autodeliverable_bare_local_file(path: str) -> bool:
    """Keep bare cached image paths from being re-uploaded on later turns."""
    return Path(str(path)).suffix.lower() not in _IMAGE_EXTS


def _caption_text_for_images(
    text_content: str,
    native_image_count: int,
) -> str | None:
    """Use final text as an image caption only when it fits native limits."""
    text = (text_content or "").strip()
    if not text or native_image_count <= 0 or len(text) > _IMAGE_CAPTION_LIMIT:
        return None
    return text
...
# Legacy adapters in the private fork imported the public name before upstream
# made this set private. Keep the alias while those adapters are migrated.
...
class MessageEvent:
    ...
    # Ephemeral nearby chat context. Adapters may populate this for the
    # per-turn system prompt; it is deliberately not persisted as user text.
...
class BasePlatformAdapter(ABC):
    ...
    def filter_local_delivery_paths(file_paths) -> List[str]:
        """Drop unsafe/noisy bare local paths and normalize accepted paths.

        Images require an explicit MEDIA directive or image markdown. This
        prevents stale generated or cached image paths from being attached to
        ...
        for file_path in file_paths or []:
            ...
            if safe_path and _is_autodeliverable_bare_local_file(safe_path):
                ...
                logger.info(
                    "Skipping bare local image auto-delivery; require "
                    "MEDIA/image markdown: %s",
                    _log_safe_path(raw),
                )
            else:
