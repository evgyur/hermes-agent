...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # Upstream's routing index and the private restart-continuation CAS
        # intentionally share this same process-root store.
        self._routing_home: Optional[Path] = self._coordination_db_path.parent
    ...
    def _routing_db(self):
        ...
        The private restart-continuation rail already captures that exact
        process-root store as ``_coordination_db``. Reuse it rather than
        opening a second logical owner for the same routing/CAS state.
        """
        try:
            return self._coordination_db
        except (AttributeError, TypeError):
            # Bare test instances may not have the private coordination
            # members; preserve upstream's fail-closed test contract.
            pinned = getattr(self, "_db_pinned", _DB_UNPINNED)
            return pinned if pinned is not _DB_UNPINNED else None
