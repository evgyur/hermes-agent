...
First-time install lives in `install.md` (clone/copy, dependencies, ffmpeg, skill registration, STT key). Don't re-run it every session; on cold start just verify:

- `ELEVENLABS_API_KEY` or `GROQ_API_KEY` resolves either from the environment or from `.env` at the video-use root. If both are missing, ask the user for one key and write it only to `.env` with `chmod 600`; never commit it.
- `ffmpeg` + `ffprobe` are on PATH.
- Python deps are installed (`uv sync` or `pip install -e .` inside the skill directory).
- Node.js + npm are available if the session needs HyperFrames or Remotion slots. HyperFrames may require Node.js 22+; use a Node 22 install if the default Node is too old.
- `yt-dlp`, HyperFrames, Remotion, and Manim are installed only on first use.
- First-use animation setup happens inside the project's `edit/animations/slot_<id>/` directory, never at the video-use root. HyperFrames can be invoked with `npx --yes hyperframes ...`; Remotion can be scaffolded with `npx create-video@latest` or installed as a project-local dependency before rendering.
- If you use Manim, add a Manim reference skill or docs under `references/` and read them before building a Manim slot.

Helpers (`helpers/transcribe.py`, `helpers/render.py`, etc.) live alongside this SKILL.md. Resolve their paths relative to the directory containing this file; keep the whole folder together when registering the skill.

## Helpers

- **`transcribe.py <video>`** — single-file transcription. Defaults to provider auto-detect from `.env` or environment variables. Use `--provider elevenlabs` when Scribe diarization/audio events are needed. `--num-speakers N` only affects ElevenLabs.
- **`transcribe_batch.py <videos_dir>`** — 4-worker parallel transcription. Supports `--provider groq|elevenlabs`; cached per source.
- **`pack_transcripts.py --edit-dir <dir>`** — `transcripts/*.json` → `takes_packed.md` (phrase-level, break on silence ≥ 0.5s).
- **`timeline_view.py <video> <start> <end>`** — filmstrip + waveform PNG. On-demand visual drill-down. **Not a scan tool** — use it at decision points, not constantly.
- **`render.py <edl.json> -o <out>`** — per-segment extract → concat → overlays (PTS-shifted) → subtitles LAST. `--preview` for 720p fast. `--build-subtitles` to generate master.srt inline.
- **`grade.py <in> -o <out>`** — ffmpeg filter chain grade. Presets + `--filter '<raw>'` for custom.

For animations, create `<edit>/animations/slot_<id>/` and spawn parallel sub-agents if your agent framework supports them; otherwise build each slot in isolation.
...
5. **Execute.** Produce `edl.json` via the editor brief, preferably in a dedicated sub-agent when available. Drill into `timeline_view` at ambiguous moments. Build animations in parallel sub-agents. Apply grade per-segment. Compose via `render.py`.
...
- **Manim** — formal diagrams, state machines, equation derivations, graph morphs. Read the local Manim reference docs you add under `references/` for depth.
...
For HyperFrames slots, scaffold the slot inside `edit/animations/slot_<id>/` with `npx --yes hyperframes init . --example blank --non-interactive`, build the HTML composition there, run the checks that fit the slot (`lint`, `validate`, and a draft render when practical), then produce the final overlay video with `npx --yes hyperframes render . -o render.mp4` or `--format webm -o render.webm` when alpha is required. If HyperFrames requires Node 22+, use a Node 22 binary/path. Point the EDL overlay `file` at the actual rendered path.
...
- Font: any installed monospaced bold font, or a bundled brand font under `assets/fonts/`
...
**Parallel sub-agent brief** — each animation is one sub-agent spawned via your agent framework. Each prompt is self-contained (sub-agents have no parent context). Include:
...
## References and assets to add for reproducibility

This public bundle intentionally ships with placeholder docs instead of private project references or brand assets. For another team to get the same quality, add:

- `references/brand-guide.md` — palette, fonts, logo rules, caption style, examples of good/bad frames.
- `references/editor-brief-template.md` — reusable brief for selecting takes and building `edl.json`.
- `references/subtitle-style.md` — chosen SRT/ASS style, casing, chunk size, margin, outline/shadow.
- `references/animation-recipes.md` — house patterns for kinetic typography, diagrams, lower thirds, UI overlays.
- `assets/fonts/` — licensed font files used by captions/overlays.
- `assets/logos/` — transparent logos/watermarks/lower-third marks.
- `assets/luts/` — optional LUTs or grade presets.
- `assets/sfx/` and `assets/music/` — licensed sounds/music beds if your workflow uses them.

Do not include API keys, raw client footage, private chat logs, local machine paths, or generated `edit/` outputs in the skill bundle.
