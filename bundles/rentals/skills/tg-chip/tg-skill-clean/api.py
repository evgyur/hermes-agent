...
class InviteBotAdminRequest(BaseModel):
    chat_id: Union[int, str]
...
@app.post("/groups/supergroup", response_model=ApiResponse)
async def create_supergroup(request: CreateSupergroupRequest):
    """Create a new supergroup, optionally enabling forum topics."""
    result = await telegram.create_supergroup(
        title=request.title,
        users=request.users,
        about=request.about or request.title,
        topics_enabled=request.topics_enabled,
    )
    return make_response(result)
...
@app.post("/groups/invite-bot-admin", response_model=ApiResponse)
async def invite_bot_admin(request: InviteBotAdminRequest):
    """Invite a bot to a chat and promote it with a named admin profile."""
    result = await telegram.invite_bot_with_admin_profile(
        chat_id=request.chat_id,
...
@app.post("/groups/topics", response_model=ApiResponse)
async def set_topics_enabled(request: TopicsRequest):
    """Enable or disable forum topics for a supergroup."""
    result = await telegram.set_topics_enabled(
        chat_id=request.chat_id,
