...
def parse_timestamp_value(raw: Union[str, int, float, datetime]) -> datetime:
    """Parse supported timestamp inputs into an aware datetime."""
    local_tz = datetime.now().astimezone().tzinfo
    if isinstance(raw, datetime):
        return raw if raw.tzinfo else raw.replace(tzinfo=local_tz)
    if isinstance(raw, (int, float)):
        return datetime.fromtimestamp(float(raw), tz=local_tz)
...
class TelegramCore:
    ...
    def _build_admin_rights(self, profile: Optional[str] = None) -> ChatAdminRights:
        """Resolve a named admin-rights profile into Telethon rights."""
        profile_name = (profile or "legacy").strip().lower()
        ...
        raise ValidationError(
            f"Invalid profile: '{profile}'. Expected one of legacy, moderator, manager, full."
        )
    ...
    async def _collect_dialogs_streaming(
        self,
        *,
        limit: int,
        archived: bool = False,
        ...
        """Collect dialogs via iter_dialogs with a hard timeout to avoid hanging get_dialogs() calls."""
        dialogs = []
        ...
        await asyncio.wait_for(_runner(), timeout=timeout_seconds)
        return dialogs
    ...
    async def get_chats(self, page: int = 1, page_size: int = 20) -> str:
        ...
        try:
            ...
            fetch_limit = max(end, page_size)
            dialogs = await self._collect_dialogs_streaming(limit=fetch_limit)
            ...
            return log_and_format_error("get_chats", e, note="dialog scan timed out")
    ...
    async def list_chats(
        ...
        try:
            await self._wait_for_rate_limit()
            dialogs = await self._collect_dialogs_streaming(limit=limit, archived=archived)
            ...
            return log_and_format_error("list_chats", e, note="dialog scan timed out", limit=limit)
        except TelethonFloodWaitError as e:
    ...
    async def send_message(
        ...
        Telegram hard-limit is ~4096 chars, so we pre-split long text.
        """
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        ...
        async def _send_chunks() -> str:
            ...
            if len(ids) == 1:
                return f"Message sent successfully. Message ID: {ids[0]}"
            return f"Message sent in {len(ids)} chunks. Message IDs: {','.join(ids)}"
        ...
        try:
            ...
            # Safety net for providers raising MessageTooLongError despite chunking.
            if e.__class__.__name__ == "MessageTooLongError":
                try:
                    entity = await self.client.get_entity(chat_id)
                    ids: List[str] = []
                    for i, chunk in enumerate(_chunk_text(message, max_len=3000)):
                        result = await self.client.send_message(
                            entity,
                            chunk,
                            reply_to=reply_to if i == 0 else None,
    ...
    async def send_file(
        ...
        If Telegram image processing fails, fallback to force_document=True.
        """
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        ...
        safe_caption = caption[:1024] if caption and len(caption) > 1024 else caption

        await self._check_message_rate_limit(chat_id)
        await self._wait_for_rate_limit()

        async def _send(force_document: bool = False):
            entity = await self.client.get_entity(chat_id)
        ...
        try:
            result = await _send(force_document=False)
            ...
            # Telegram sometimes rejects image processing; send as document instead.
            if e.__class__.__name__ == "ImageProcessFailedError":
                try:
                    result = await _send(force_document=True)
                    return f"File sent as document. Message ID: {result.id}"
                except Exception as retry_e:
                    return log_and_format_error("send_file", retry_e, chat_id=chat_id)
    ...
    async def create_supergroup(
        self,
        title: str,
        users: Optional[List[Union[int, str]]] = None,
        about: str = "",
        topics_enabled: bool = False,
    ) -> str:
        """Create a new supergroup, optionally enabling forum topics."""
        users = users or []
        users, error = validate_ids("users", users)
        if error:
            return error
        ...
        try:
            ...
            if users:
                user_entities = []
                for user_id in users:
                    user_entities.append(await self.client.get_entity(user_id))
                await self.client(
            ...
            return json.dumps(
                {
                    ...
                    "invited_users": len(users),
                },
                indent=2,
                default=json_serializer,
            )
        except Exception as e:
            return log_and_format_error("create_supergroup", e, title=title)
    ...
    async def set_topics_enabled(
        self,
        chat_id: Union[int, str],
        enabled: bool,
        tabs: Optional[bool] = None,
    ) -> str:
        """Enable or disable forum topics for a supergroup."""
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        ...
        try:
            ...
            if not isinstance(entity, Channel):
                return "Topics are supported only for channels and supergroups."

            tabs_enabled = enabled if tabs is None else tabs
            await self.client(
            ...
            return log_and_format_error("set_topics_enabled", e, chat_id=chat_id)
    ...
    async def promote_admin(
        ...
        try:
            ...
            if isinstance(entity, Channel):
                rights = self._build_admin_rights(profile)
                await self.client(
    ...
    async def invite_bot_with_admin_profile(
        self,
        chat_id: Union[int, str],
        ...
        """Invite a bot to a chat and promote it with a named admin profile."""
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        bot_user_id, error = validate_ids("bot_user_id", bot_user_id)
        if error:
            return error

        invited = True
        try:
            invite_result = await self.invite_to_group(chat_id=chat_id, user_ids=[bot_user_id])
            if invite_result.startswith("An error occurred"):
                return invite_result
        except UserAlreadyParticipantError:
            invited = False
        except Exception as e:
            return log_and_format_error(
                "invite_bot_with_admin_profile",
                e,
                chat_id=chat_id,
                bot_user_id=bot_user_id,
            )

        promote_result = await self.promote_admin(
            chat_id=chat_id,
            user_id=bot_user_id,
            title=title,
            profile=profile,
        )
        if promote_result.startswith("An error occurred") or promote_result.startswith("Invalid profile"):
    ...
    async def get_join_requests(self, chat_id: Union[int, str], limit: int = 100) -> str:
        ...
        try:
            ...
            if not result.importers:
                return json.dumps({"importers": [], "count": 0}, indent=2)

            importers = []
            for imp in result.importers:
                importers.append({
                    "user_id": imp.user_id,
                    "username": imp.user.username if imp.user else None,
                    "first_name": imp.user.first_name if imp.user else None,
                    "last_name": imp.user.last_name if imp.user else None,
                    "date": imp.date.isoformat() if imp.date else None,
                    "approved": imp.approved,
                    "request_id": imp.request_id
                })

            return json.dumps({"importers": importers, "count": len(importers)}, indent=2, default=json_serializer)
        except Exception as e:
            return log_and_format_error("get_join_requests", e, chat_id=chat_id)

    async def approve_join_request(self, chat_id: Union[int, str], user_id: Union[int, str]) -> str:
        """Approve a join request for a user."""
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        ...
        try:
            ...
            return json.dumps({"success": True, "chat_id": chat_id, "user_id": user_id, "action": "approved"}, indent=2)
        except Exception as e:
            return log_and_format_error("approve_join_request", e, chat_id=chat_id, user_id=user_id)

    async def create_invite_link(self, chat_id: Union[int, str], usage_limit: int = 1, title: str = "") -> str:
        """Create a new invite link with usage limit."""
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        ...
        try:
            ...
            result = await self.client(
                functions.messages.ExportChatInviteRequest(
                    ...
                    title=title or f"Invite for {chat_id}"
                )
            )
    ...
    async def find_nearest_media_message(
        self,
        chat_id: Union[int, str],
        target_ts: Union[str, int, float],
        from_id: Optional[Union[int, str]] = None,
        window_seconds: int = 180,
        limit: int = 50,
        require_empty_text: bool = True,
    ) -> str:
        """Find the nearest media message in a chat around a target timestamp."""
        chat_id, error = validate_ids("chat_id", chat_id)
        if error:
            return error
        from_id, error = validate_ids("from_id", from_id)
        if error:
            return error
        try:
            target_dt = parse_timestamp_value(target_ts)
        except Exception as e:
            return log_and_format_error(
                "find_nearest_media_message",
                e,
        ...
        try:
            try:
                ...
                async for dialog in self.client.iter_dialogs():
                    ...
                    if dialog_id is not None and str(dialog_id) == str(chat_id):
                        entity = dialog.entity
                        break
                if entity is None:
                    raise
            offset_date = target_dt + timedelta(seconds=max(window_seconds, 0))
            candidates = []
            async for msg in self.client.iter_messages(entity, offset_date=offset_date, limit=limit):
                if not msg or not msg.date:
                    continue
                delta = abs((msg.date - target_dt).total_seconds())
                if delta > window_seconds:
                    continue
                if not msg.media:
                    continue
                if require_empty_text and (msg.message or "").strip():
                    continue
                if from_id is not None:
                    msg_from_id = utils.get_peer_id(msg.from_id) if msg.from_id else None
            ...
            if not candidates:
                return json.dumps({"success": False, "error": "No matching media message found"})
            candidates.sort(key=lambda item: (item["delta_seconds"], -item["message"]["id"]))
            best = candidates[0]
            return json.dumps({
                "success": True,
            ...
            return log_and_format_error(
                ...
                limit=limit,
                require_empty_text=require_empty_text,
            )
    ...
    async def export_messages(
        ...
        try:
            ...
            if not out_path:
                ...
                out_dir = Path(os.getenv("TELEGRAM_EXPORT_DIR", "/opt/telegram-chip/exports"))
