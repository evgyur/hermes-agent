        ...
        if not all(hasattr(os, name) for name in ("O_DIRECTORY", "O_NOFOLLOW", "O_CLOEXEC")):
            ...
            while True:
                ...
                if current.is_symlink() or reparse:
                    raise ValueError("Primary image path contains a link or reparse point")
...
def register(ctx, *, required: bool = False) -> None:
    """Register the Human20 Keys image and Groq transcription backends."""
    from .transcription import Human20KeysGroqTranscriptionProvider

    image = Human20KeysOpenAICodexImageGenProvider()
    transcription = Human20KeysGroqTranscriptionProvider()
