...
- Do not ask the operator to send `/goal` again if the standing-goal wrapper is active.

## Good visible wording

`SUPERGOAL_PHASE_VERIFY` should say explicitly that this is transcript bookkeeping for an already-recorded phase, then list the evidence paths and pass/fail criteria. This makes the evaluator and the operator see why no new product edits happened in this turn.
