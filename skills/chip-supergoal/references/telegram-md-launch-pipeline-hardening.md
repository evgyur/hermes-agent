...
See also `references/telegram-review-file-delivery-gate.md`: the operator's current default expectation is that the three human-facing SuperGoal planning files (`THINKING.md`, `ROADMAP.md`, `LAUNCH_GOAL.md`) are sent as native Telegram files with the TЗ/review package before any start/dispatch prompt. Delivery must be scripted and receipt-backed, not dependent on agent memory.
...
- the operator replies `/goal` only to `LAUNCH_GOAL.md` unless an explicit button launch path is implemented and verified.
...
If the operator calls out instability in the launch rail, stop promising. Run Shaw-style review over the whole pipeline: Telegram hydration, extraction, GoalManager state, compression migration, restart proof, artifacts/skill docs, and tests. Fix guardrails first, then relaunch.
