...
4. Do not re-run final audit, deploy smoke, build, or live probes unless the operator explicitly asks for fresh verification.
...
8. Only answer substantively again if the operator adds a new human question, asks for fresh verification, reports a bug, or provides a new goal. Then answer that new request, not the stale continuation wrapper.
