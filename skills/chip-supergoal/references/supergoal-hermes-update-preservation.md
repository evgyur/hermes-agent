...
1. The live Hermes checkout commits the code and pushes it to the deployment's private Hermes fork (`private/main`) when the user wants it shipped, not just locally verified.
...
Do not tell the user/operator “it is safe across updates” if the fix exists only as dirty files in `<hermes-agent-install>`.
