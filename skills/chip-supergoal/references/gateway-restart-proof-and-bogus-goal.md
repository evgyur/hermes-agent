...
sudo systemd-run \
  ...
  --collect \
  /bin/bash -lc 'sleep 20; exec <hermes-agent-install>/venv/bin/python <hermes-home>/scripts/hermes_gateway_restart_probe.py'
```

The proof script should:

- set `HERMES_HOME=<hermes-home>` for subprocess probes;
