...
class TestRunConversation:
    ...
    def test_codex_useful_byte_watchdog_final_failure_is_chat_silent(self, agent):
        self._setup_agent(agent)
        agent.provider = "openai-codex"
        agent.model = "gpt-5.5"
        agent.api_mode = "codex_responses"
        ...
        def fail_codex_watchdog(_api_kwargs):
            raise TimeoutError(
                "Codex stream produced no useful bytes within 12s "
                "(TTFB threshold: 12s). Codex backend appears to be silently "
                "rejecting 'gpt-5.5' on chatgpt.com/backend-api/codex"
            )

        with (
            patch.object(agent, "_interruptible_api_call", side_effect=fail_codex_watchdog),
        ...
        assert result["error"] == ""
        assert not any("API failed after" in s for s in statuses)
