...
def test_default_restart_enters_resumable_stop_immediately():
    """A planned restart must not wait for a long autonomous turn.

    ``stop()`` persists exact continuation authority before it interrupts the
    agent, so the safe and user-visible default is to enter that path
    immediately. Deployments that deliberately prefer finish-before-restart
    can still opt in with a positive override.
    """
    assert DEFAULT_GATEWAY_RESTART_AFTER_TURN_TIMEOUT == 0
    assert resolve_restart_exit_wait_budget(
        0, DEFAULT_GATEWAY_RESTART_AFTER_TURN_TIMEOUT, headroom=30
    ) == 30
