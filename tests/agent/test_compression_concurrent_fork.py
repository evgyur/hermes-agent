...
def test_overlapping_agent_compressions_keep_one_cancellable_active_fence(
    tmp_path: Path,
) -> None:
    """A second attempt cannot replace the fence of an in-flight attempt."""
    from agent.conversation_compression import CompressionCommitFence

    db = SessionDB(db_path=tmp_path / "state.db")
    session_id = "OVERLAPPING_ACTIVE_FENCE"
    db.create_session(session_id, source="tui")
    agent = _build_agent_with_db(db, session_id)
    agent.compression_in_place = True
    agent._cached_system_prompt = "sys"
    ...
    first = threading.Thread(target=_attempt, daemon=True)
    second = threading.Thread(target=_attempt, daemon=True)
    first.start()
    assert first_started.wait(timeout=2)
    first_fence = vars(agent).get("_active_compression_commit_fence")
    second.start()
    time.sleep(0.05)
    assert calls == 1
    assert vars(agent).get("_active_compression_commit_fence") is first_fence

    agent.hard_interrupt("cancel overlapping compression attempts")
    release_first.set()
    ...
    assert all(result[0] is messages for result in results)
    archive_spy.assert_not_called()
    assert "_active_compression_commit_fence" not in vars(agent)
    assert db.get_compression_lock_holder(session_id) is None


def test_agent_wrapper_forwards_critical_ineffective_guard_bypass(
    tmp_path: Path,
) -> None:
    """Gateway hygiene's narrow breaker bypass survives the AIAgent seam."""
    from agent.conversation_compression import CompressionCommitFence

    db = SessionDB(db_path=tmp_path / "state.db")
    session_id = "FORWARD_INEFFECTIVE_BYPASS"
    db.create_session(session_id, source="tui")
    agent = _build_agent_with_db(db, session_id)
    agent._cached_system_prompt = "sys"
    ...
    assert wrapped.call_args.kwargs["bypass_ineffective_guard"] is True
