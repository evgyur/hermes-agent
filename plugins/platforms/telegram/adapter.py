...
def _looks_like_inline_tg_preview(text: str) -> bool:
    """Identify finished Telegram drafts that must use the external bridge."""
    if not text or not text.strip():
        return False
    stripped = text.strip()
    if _INLINE_TG_OPERATOR_PREFIX_RE.search(stripped) or "➊" in stripped or "┈" in stripped:
        return False
    first_line = stripped.splitlines()[0].strip()
    has_html_heading = first_line.startswith("<b>") and "</b>" in first_line
    has_tg_block_separator = "⠀" in stripped
    has_source_label = bool(re.search(r"(?im)^\\s*(источник|source)\\s*:", stripped))
    has_markdown_source = bool(
        re.search(r"(?im)^\\s*(источник|source)\\s*:\\s*\\[[^\\]]+\\]\\(https?://", stripped)
    )
    has_post_body = len([line for line in stripped.splitlines() if line.strip()]) >= 4
    ...
    return bool(
        short_title
        and has_tg_block_separator
        and has_post_body
        and (has_source_label or has_markdown_source)
    )
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        if isinstance(rich_chat_ids, str):
            try:
                ...
                rich_chat_ids = [part.strip() for part in rich_chat_ids.split(",") if part.strip()]
    ...
    def _is_bad_request_error(error: Exception) -> bool:
        ...
        # Some embedders/tests expose optional PTB exception attributes through
        # a MagicMock module. Those placeholders are not transport failures.
    ...
    def _looks_like_network_error(error: Exception) -> bool:
        ...
        try:
            ...
            semantic_types = tuple(
                exc_type
                for exc_type in (BadRequest, InvalidToken, Forbidden, RetryAfter)
                if isinstance(exc_type, type)
            )
            transport_types = tuple(
                exc_type
                for exc_type in (NetworkError, TimedOut)
                if isinstance(exc_type, type)
            )
            if semantic_types and isinstance(error, semantic_types):
                return False
            if transport_types and isinstance(error, transport_types):
                return True
        except ImportError:
            pass
    ...
    async def _probe_pending_updates(self, bot, probe_timeout: float) -> None:
        ...
        try:
            ...
            # Propagate transport failures to the heartbeat's shared reconnect
            # classifier. Swallowing here leaves getUpdates wedged indefinitely.
            raise
            ...
            for i, chunk in enumerate(chunks):
                ...
                for _send_attempt in range(3):
                    try:
                        ...
                        if (
                            business_kwargs
                            and "business_peer_invalid" in str(send_err).lower()
                        ):
                            business_kwargs = {}
                            continue
    ...
    def _effective_message_thread_id(cls, message: Message) -> Optional[str]:
        ...
        chat_id_text = str(getattr(chat, "id", "") or "") if chat else ""
        raw = getattr(message, "message_thread_id", None)
        is_topic_message = bool(getattr(message, "is_topic_message", False))
        is_group = chat_type in ("group", "supergroup") or (
            chat_id_text.startswith("-") and chat_type not in {"channel", "private"}
        )
        is_forum_group = is_group and getattr(chat, "is_forum", False) is True
        if raw is not None:
            if is_forum_group or (is_group and is_topic_message):
                return str(raw)
    ...
    def _inject_telegram_text_links(text: str, entities: Optional[List[Any]]) -> str:
        ...
        for entity in entities:
            ...
            if not url or url in text or not any(
                marker in entity_type for marker in ("text_link", "texturl")
            ):
                continue
            try:
                ...
                offset, length = 0, 0
            label = text[offset:offset + length].strip() if length else "link"
            key = (label or "link", str(url))
            if key not in seen:
                seen.add(key)
                links.append(key)
        if not links:
    ...
    def _message_text_with_hidden_links(cls, message: Any) -> str:
        ...
        if caption:
            return cls._inject_telegram_text_links(
    ...
    def _build_message_event(
        ...
        if telegram_chat_type in {"group", "supergroup"} or (
            chat_id_text.startswith("-")
            and telegram_chat_type not in {"channel", "private"}
        ):
            chat_type = "group"
        ...
        if message.reply_to_message:
            ...
            if quote_text:
                ...
                reply_to_text = self._message_text_with_hidden_links(
